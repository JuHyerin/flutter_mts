...
class KisStockPurchase extends KisStockData{
  ...
  late List<String> askPriceList; // 매도호가 (주식 판매하고자 하는 금액), ASKP[1~10]
  late List<String> bidPriceList; // 매수호가 (주식 구매하고자 하는 금액), BIDP[1~10]
  late List<String> askRestQuantityList; // 매수호가 잔량, ASKP_RSQN[1~10]
  late List<String> bidRestQuantityList; //매도호가 잔량, BIDP_RSQN[1~10]
  ...
  late String anticipatedCnVersusSign; // 예상 체결 대비 부호, ANTC_CNTG_VRSS_SIGN ( 1:상한, 2:상승, 3:보합, 4:하한, 5:하락)
...
/* 실시간 주식 체결가 */
class KisStockCntg extends KisStockData {
  late String MKSC_SHRN_ISCD; // 유가증권 단축 종목코드
  late String STCK_CNTG_HOUR; // 주식 체결 시간
  late String stockCurPrice; // 주식 현재가
  ...
  KisStockCntg.parse(String stringData){
    List<String> list = stringData.split('^');
    MKSC_SHRN_ISCD = list[0];
    STCK_CNTG_HOUR = list[1];
    stockCurPrice = list[2];
